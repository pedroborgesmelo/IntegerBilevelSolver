...
std::string generateMpsDeltaEmbedding(double delta) {
	...
	model.add(x2a <= 10.0);
	// add objective
	IloObjective obj = IloMinimize(env , x1 + x1a);
	model.add(obj);
	// create cplex model
...
bool myIsFeasibleForAbstractConstraintOpt(IloNumArray wholeIncumbent) {
	return wholeIncumbent[1] >= 5.0 && wholeIncumbent[3] >= 5.0;
}
...
int main(int argc, char* argv[]) {
	...
	std::cout << std::endl;
	// test sol
	if(fabs(solFoundVec[0] - 3.0) <= 0.0001 && fabs(solFoundVec[2] - 4.0) <= 0.0001 &&
		solFoundVec[1] >= 5.0 && solFoundVec[1] <= 10.0 && 
		solFoundVec[3] >= 7.0 && solFoundVec[3] <= 10.0) {
		std::cout << std::endl;
